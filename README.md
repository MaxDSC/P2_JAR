...
4. Для выполнения четвертого квеста создал пакет`maxdsc.quests.func` и класс `Functions.java`.
- Для округления числе пользовался форматером String `String.format("%.7f", agnesi);`, где %.7f обозначает округлить до семи знаков после запятой, в случаеесли седьмой знак 5, то округляет в большую сторону.
- Для более лучшего вычисления вещественных чисели их округления в java лучше использовать класс `BigDecimal`.
