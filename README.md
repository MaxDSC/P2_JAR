...
3. [Ход работы](#3).\
   3.1. [Как работать с JAR](#31)\
   3.2. [Решение первого квеста](#32)\
   3.3. [Решение второго квеста](#33)\
   3.4. [Решение третьего квеста](#34)\
   3.5. [Решение четвертого квеста](#35)\
   3.6. [Решение пятого квеста](#36)
...
***Дополнить класс Functions.java методом отрисовки так, чтобы он отрисовывал в терминале графики рассчитываемых функций при помощи символа "\*"(звездочка). Масштаб по оси абсцисс — 42 отсечки, масштаб оси ординат — 21. Расположение и поворот координатых осей не принципиальны. ==***

## <a name="3"></a> 3. Ход работы
### <a name="31"></a> Как работать с JAR
...
### <a name="32"></a> Решение первого квеста
...
3. Для компиляции добавил в *Makefile* к `javac` параметр `-sourcepath src/main/java`, который указывает где брать файлы исходного кода для компиляции связанных между собой классов, так как в классе `Main.java` идет обращение к другим классам.
4. Упростил *Makefile*, так чтобы везде использовался относительный путь.
5. Переместил библиотеки для проверки стиля [(см. п. 1.2)](#12) и форматирования кода по стилю в папку *lib*, в *Makefile* также изменил их вызов именно из этой папки
### <a name="33"></a> Решение второго квеста
1. Для выволнения второго квеста создал пакет`maxdsc.quests.encrypt` и класс `Decode.java`.
2. Нужно быть внимательным в случае, если сначала использовать, для проверки режима кодирования/декодирования, `.nextInt`, а потом `.nextLine`, то не получится считать последнюю строку, так как после `.nextInt` остается кэш с символом переноса, то есть консоль не будет просить ввести строку в консоль.
3. Для кодирования символов в методе `public void encode()`  использовал форматированный вывод `System.out.printf("%x ", decSymbol);` где `%x` указывает, что нужно переделать на выводе число из десятичной системы в шестнадцатиричную.
4. Для декодирования строки, состоящей из подстрок двух символов в шестнадцатиричной системе, разделенных пробелами, в методе `public void decode()` использовал `String twoSymbols = new StringBuilder().append(symbol1).append(symbol2).toString();`, которая преобразует два символа, которые мы извлекаем из исходной строки в подстроку, обозначающую число в 16-ой системе, а затем переводим эту подстроку в число в десятичной системе с помощью  `Integer decSymbol = Integer.parseInt(twoSymbols, 16);`, и для вывода уже одного символа в нормальном виде используем форматированный вывод: `System.out.printf("%c ", decSymbol.intValue());`, где `%c` обозначает, что нужно преобразовать число в символ.
### <a name="34"></a> Решение третьего квеста
1. Для выполнения третьего квеста создал пакет`maxdsc.quests.fibonacci` и класс `Fibonacci.java`.
2. Рекурсивно вычисление происходит с помощью следующей строчки кода в методе `private int calcFib(int n)`:
```
result = calcFib(n - 1) + calcFib(n - 2);
```
### <a name="35"></a> Решение четвертого квеста
1.  Для выполнения четвертого квеста создал пакет`maxdsc.quests.func` и класс `Functions.java`.
2. Для округления числе пользовался форматером String `String.format("%.7f", agnesi);`, где %.7f обозначает округлить до семи знаков после запятой, в случае если седьмой знак 5, то округляет в большую сторону.
3. Для более лучшего вычисления вещественных чисели и их округления в java лучше использовать класс `BigDecimal`.
### <a name="36"></a> Решение пятого квеста
Для выполнения пятого квеста в классе `Functions.java`, создал три метода:
1. `private double calcStepForY()` - для вычисления отсечек графика для y (шаг графика по у).
2. `private void printGraphToConsole(UnaryOperator<Double> func)` - для вывода на экран одного графика в зависимости от переданного в функциональный интерфейс `UnaryOperator` метода, который вычисляет значения функции. Данный функциональный интерфейс имеет метод `apply`, который принимает один параметр (у меня Double), и возвращает объект того же типа, что и параметр. Использовал данный интерфейс, чтобы писать меньше кода, потому что в методе `printGraphToConsole` нужно переписать лишь одну строку для вывода разнообразных графиков.
3. `public void printAllGraphToConsole()` - для вывода всех трех графиков функций. Здесь определял какие методы будут реализованы через функциональный интерфейс, а затем для каждого интерфейса отрисовал график.
